...
interface Props { receiverName?: string; senderName?: string; activityName?: string; activityType?: string; proposedTime?: string; message?: string }

const ActivityInviteEmail = ({ receiverName, senderName, activityName, activityType, proposedTime, message }: Props) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>{senderName || 'Someone'} invited you to {activityName || 'an activity'}!</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={header}><Heading style={h1}>🎉 Activity Invite!</Heading></Section>
        <Section style={body}>
          <Text style={text}>Hey {receiverName || 'there'}! 👋</Text>
          <Text style={text}><strong style={{color:'#E04F3E'}}>{senderName || 'A friend'}</strong> wants to do something fun with you!</Text>
          <Section style={card}>
            <Text style={cardTitle}>{activityName || 'Activity'}</Text>
            {proposedTime ? <Text style={cardText}>📅 When: {proposedTime}</Text> : null}
            {activityType ? <Text style={cardText}>🎯 Type: {activityType}</Text> : null}
            {message ? <><Text style={msgLabel}>💬 Message:</Text><Text style={msgText}>"{message}"</Text></> : null}
          </Section>
          <Button style={button} href="https://pulseos.tech/app/friends">View Invite →</Button>
          <Text style={hint}>You can accept, decline, or suggest a different time!</Text>
        </Section>
        <Section style={footer}><Text style={footerText}>A friend invited you to an activity.</Text></Section>
      </Container>
    </Body>
  </Html>
)
...
const cardText = { color: '#1e293b', fontSize: '14px', margin: '0 0 8px', fontWeight: '500' as const }
