...
interface Props { inviterName?: string }

const AppInviteEmail = ({ inviterName }: Props) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>{inviterName || 'Someone'} invited you to join PulseOS! 🚀</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={header}><Heading style={h1}>⚡ You're Invited to PulseOS</Heading></Section>
        <Section style={body}>
          <Text style={text}>Hey there! 👋</Text>
          <Text style={text}><strong style={{color:'#E04F3E'}}>{inviterName || 'A friend'}</strong> thinks you'd love PulseOS - your personal life dashboard.</Text>
          <Section style={featureBox}><Text style={featureTitle}>🌤️ Weather & Location</Text><Text style={featureText}>Real-time weather updates</Text></Section>
          <Section style={featureBox}><Text style={featureTitle}>👥 Connect with Friends</Text><Text style={featureText}>Stay connected and plan activities</Text></Section>
          <Section style={featureBox}><Text style={featureTitle}>🤖 AI Assistant</Text><Text style={featureText}>Personalized insights and recommendations</Text></Section>
          <Button style={button} href="https://pulseos.tech/auth">Join PulseOS Now →</Button>
        </Section>
        <Section style={footer}><Text style={footerText}>You're receiving this because {inviterName || 'a friend'} invited you.</Text></Section>
      </Container>
    </Body>
  </Html>
)
